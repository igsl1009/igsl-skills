# IGSL SKILL META-SCAN | 2026-04-16 | 14 nodes

## HARD SKILLS (auto-loaded every session)
  [META-05     ] auto-improve             280tok  h=0.95  Always-on. Session journal, captures corrections, propo
  [META-07     ] memory-system            180tok  h=0.95  Always-on. Memory write protocol: node.py add/update/cl
...
## SOFT-HOOK SKILLS (load when keywords match)
  ...
  [META-03     ] pipeline-builder         360tok  h=0.71  triggers:[build pipeline design pipeline scaffold execution order project structure]
  [META-04     ] gap-filler               340tok  h=0.70  triggers:[build new skill spec new node fill gap create skill tailor made]
  [META-06     ] variant-manager          320tok  h=0.69  triggers:[variant version different context patch overlay]
